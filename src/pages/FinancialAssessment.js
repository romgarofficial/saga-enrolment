...
export default function FinancialAssessment(){
    ...
    const refreshAcademicAssessment = () => {
        getAssessment2Application(currentPage);
      };


    useEffect(() => {
        document.title = 'SAGA - Financial Assessment';
        getAssessment2Application(currentPage);
      }, [currentPage]);
...
const getAssessment2Application = (page) => {
    ...
    const approveApplication = (id) => {
        fetch(`${process.env.REACT_APP_ONE_SAGA_URL}/enrollment/financial-assessment/${id}`, {
            ...
            .then(data => {
                ...
                if(data == "1"){
                    Swal.fire({
                        ...
                        icon: "success",
                        text: `${firstName} ${middleName} ${lastName} is now ready for Final Verification.`
                    });
      ...
      const renderTableRows = () => {
        if (!allAdmission || allAdmission.length === 0) {
            ...
            return allAdmission.slice(startIndex, endIndex).map((assessment2, index) => (
                <tr key={assessment2._id}>
                    <td>{startIndex + index + 1}</td>
                    <td>{`${assessment2.firstName} ${assessment2.middleName} ${assessment2.lastName}`}</td>
                    <td>{assessment2.lrn}</td>
                    <td>{assessment2.gradeLevelToEnroll}</td>
                    <td>{assessment2.strandToEnroll}</td>
                    <td>{assessment2.mobileNumber}</td>
                    <td>{assessment2.studentStatus}</td>
                    <td className="text-center">
                        <ButtonGroup>
                            <Button variant="primary" size="sm" onClick={() => openViewModal(assessment2._id)}>View</Button>
                            <Button variant="success" size="sm" onClick={() => openApproveModal(assessment2._id)}>Approve</Button>
            ...
            <Container fluid className="d-flex bg-lightgray flex-wrap p-2 p-lg-2 flex-column text-center">
                <h1 className="display-5 fw-bold ms-5 my-2">FINANCIAL ASSESSMENT</h1>
                    ...
                    <Container fluid className="d-flex flex-column flex-lg-row ">
                        <Button clas variant="warning"  className="px-5 mx-0 mx-lg-0 my-2 my-lg-3" onClick={() => getAssessment2Application()}> <BsArrowClockwise size={20} className="p-0 m-0 mx-2" />Refresh</Button>
                    </Container>
